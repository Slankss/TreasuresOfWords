...
class TranslatedAdapter(private val translatedArrayList : ArrayList<Question>,var mContext : Context)
    : RecyclerView.Adapter<TranslatedAdapter.HolderView>() {
    ...
    override fun onBindViewHolder(holder: HolderView, position: Int) {
        ...
        holder.binding.apply {
            ...
            var current = translatedArrayList[position]

            txtPosition.text = (position+1).toString()

            var text = when(matchingList[position]){
                null -> current.translated
                else -> {
                     current.translated +"  ("+(matchingList[position])+")"
                }
            }
            var state : Boolean? = translatedArrayList[position].state
    ...
    override fun getItemCount(): Int {
        return translatedArrayList.size
    }
