...
class QuizViewModel(private var auth : FirebaseAuth, private var db : FirebaseFirestore, ) : ViewModel() {
    ...
    var learnedList = MutableLiveData<ArrayList<HashMap<String,Any>>>()
            ...
            db.collection("Word").document(uid)
                .get().addOnCompleteListener { task ->
                    if(task.isSuccessful){
                        ...
                        val learnArrayList = arrayListOf<HashMap<String,Any>>()
                        if(document != null && document.data != null && document.exists()){
                            ...
                            wordListInDb.forEachIndexed { index, it ->
                                ...
                                if(quizTime.isNotEmpty() ){
                                    ...
                                    when(repeatTime){
                                        3 -> {
                                            if(diff >= 1) wordListArray.add(hashMap)
                                        }
                                        4 -> {
                                            if(diff >= 2) wordListArray.add(hashMap)
                                        }
                                        5 -> learnArrayList.add(hashMap)
                                        else -> {
                                            wordListArray.add(hashMap)
                                        }
                                    }
                        ...
                        learnedList.value = learnArrayList
                    }
